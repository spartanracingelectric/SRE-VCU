...
CanManager* CanManager_new(ubyte2 busSpeed[CAN_CHANNELS], ubyte1 read_messageLimit[CAN_CHANNELS], ubyte1 write_messageLimit[CAN_CHANNELS], ubyte4 defaultSendDelayus) //ubyte4 defaultMinSendDelay, ubyte4 defaultMaxSendDelay)
{
    ...
    for (int i=0; i < 2; ++ i){
        ...
        //, the frame size
        me->ioErr_fifoInit_R[i] = IO_CAN_ConfigFIFO(&me->readHandle[i], IO_CAN_CHANNEL_0 + i, read_messageLimit[i], IO_CAN_MSG_READ, IO_CAN_STD_FRAME, 0, 0);
        me->ioErr_fifoInit_W[i] = IO_CAN_ConfigFIFO(&me->writeHandle[i], IO_CAN_CHANNEL_0 + i, write_messageLimit[i], IO_CAN_MSG_WRITE, IO_CAN_STD_FRAME, 0, 0);
...
IO_ErrorType CanManager_send(CanManager* me, CanChannel channel, IO_CAN_DATA_FRAME canMessages[], ubyte1 canMessageCount)
{
    ...
    for (messagePosition = 0; messagePosition < canMessageCount; messagePosition++)
    {
        ...
        if (firstTimeMessage)
        {
            lastMessage = (AVLNode*)malloc(sizeof(AVLNode));
            if (lastMessage == NULL)
            {
                return IO_E_NULL_POINTER;
            }
            me->canMessageHistory[outboundMessageID] = lastMessage;
            lastMessage->timeBetweenMessages_Min = 25000;
            lastMessage->timeBetweenMessages_Max = 125000;
            lastMessage->required = TRUE;
            for (ubyte1 i = 0; i <= 7; i++) { lastMessage->data[i] = 0; }
            lastMessage->lastMessage_timeStamp = 0;
        }
        ...
        for (ubyte1 dataPosition = 0; dataPosition < canMessages[messagePosition].length; dataPosition++)
        {
            ...
            if (oldData != newData) { dataChanged = TRUE; }
    ...
    if (messagesToSendCount > 0)
    {
        ...
        *((channel == CAN0_HIPRI) ? &me->ioErr_write[0] : &me->ioErr_write[1]) = sendResult;
        ...
        if ((channel == CAN0_HIPRI ? me->ioErr_write[0] : me->ioErr_write[1]) == IO_E_OK)
        {
            ...
            {
                lastMessage = me->canMessageHistory[messagesToSend[messagePosition].id];
                for (ubyte1 dataPosition = 0; dataPosition < messagesToSend[messagePosition].length; dataPosition++)
                {
                    lastMessage->data[dataPosition] = messagesToSend[messagePosition].data[dataPosition];
                }
                IO_RTC_StartTime(&lastMessage->lastMessage_timeStamp);
            }
        }
    }
    return sendResult;
...
void canOutput_sendDebugMessage0(CanManager* me, TorqueEncoder* tps, BrakePressureSensor* bps, InstrumentCluster* ic, BatteryManagementSystem* bms, SafetyChecker* sc, _Powertrain *powertrain, SDiff *sdiff)
{
    ...
    canMessages[canMessageCount - 1].id = 0x500;
    ...
    byteNum = 0;
    canMessages[canMessageCount - 1].id_format = IO_CAN_STD_FRAME;
    canMessages[canMessageCount - 1].id = 0x501;
    canMessages[canMessageCount - 1].data[byteNum++] = throttlePercent;
    ...
    byteNum = 0;
    canMessages[canMessageCount - 1].id_format = IO_CAN_STD_FRAME;
    canMessages[canMessageCount - 1].id = 0x502;
    ...
    canMessages[canMessageCount - 1].data[byteNum++] = (sbyte4)(SDiff_getSharedMultiplier(sdiff) * 100); // Convert to percentage
    canMessages[canMessageCount - 1].data[byteNum++] = (sbyte4)(SDiff_getSharedMultiplier(sdiff) * 100) >> 8;
    canMessages[canMessageCount - 1].data[byteNum++] = (sbyte4)(SDiff_getSharedMultiplier(sdiff) * 100) >> 16;
    canMessages[canMessageCount - 1].data[byteNum++] = (sbyte4)(SDiff_getSharedMultiplier(sdiff) * 100) >> 24;
    ...
    //0x505 or 1285: Safety Checker
    canMessageCount++;
    byteNum = 0;
    canMessages[canMessageCount - 1].id_format = IO_CAN_STD_FRAME;
    canMessages[canMessageCount - 1].id = 0x505;
    ...
    //0x506 or 1286: LV Battery (24V)
    canMessageCount++;
    byteNum = 0;
    canMessages[canMessageCount - 1].id = 0x506;
    canMessages[canMessageCount - 1].id_format = IO_CAN_STD_FRAME;
    ...
    //0x507 or 1287: MCM RTD Status
    canMessageCount++;
    byteNum = 0;
    canMessages[canMessageCount - 1].id = 0x507;
    canMessages[canMessageCount - 1].id_format = IO_CAN_STD_FRAME;
    ...
    //0x508 or 1288: BMS Loopback Test
    canMessageCount++;
    byteNum = 0;
    canMessages[canMessageCount - 1].id = 0x508;
    canMessages[canMessageCount - 1].id_format = IO_CAN_STD_FRAME;
...
{
    // EXTENDED CAN BUS MESSAGES (CAN1)
...
void canOutput_sendBMSCommands(CanManager *me, BatteryManagementSystem *bms)
{
    ...
    me->ioErr_write[1] = IO_CAN_WriteFIFO(me->writeHandle[1], canMessages, 1);
}
